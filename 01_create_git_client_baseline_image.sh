...
[[ ${__IMAGE_EXISTS} -eq ${__YES} ]]  \
  && {
    echo "______ Image ${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION} exists"
    __REBUILD_IMAGE=${__NO}
  } \
  || {
    echo "______ Image ${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION} does not exist"
...
if [[ ${__REBUILD_IMAGE} -eq ${__YES} ]]; then
  ...
  echo "______ Image ${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION} built"
fi
...
if [[ $STS -eq ${__YES} ]]; then
  ...
  echo "______ Container ${__GIT_CLIENT_CONTAINER_NAME} stopped and removed"
else
  echo "______ Container ${__GIT_CLIENT_CONTAINER_NAME} does not exist"
fi
...
if [[ $STS -eq ${__DONE} ]]; then
  ...
  fn__CommitChangesStopContainerAndSaveImage   \
    ...
    "${__GIT_CLIENT_IMAGE_NAME}" \
    "${__GIT_CLIENT_IMAGE_VERSION}"
  echo "______ Commited changes to ${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION} and Stopped container ${__CONTAINER_NAME}"
  ...
  if [[ ${__PUSH_TO_REMOTE_DOCKER_REPO} == ${__YES} ]]; then
    ...
    echo "______ Image tagged and pushed to repository as ${__DOCKER_REPOSITORY_HOST}/${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION}" 
  else
    echo "______ On user request on user request image ${__GIT_CLIENT_IMAGE_NAME}:${__GIT_CLIENT_IMAGE_VERSION} has NOT been pushed to Docker repository ${__DOCKER_REPOSITORY_HOST}" 
  fi
else
  ${__INDUCE_ERROR}
fi
